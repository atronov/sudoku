// this code has just copy-pasted from Internet
// it will be rewritten on TypeScript later
 ...
 */
const search = function(h, s, addSulution) {
    if (h.R == h) {
        addSulution(s);
    } else {
...
export const reduceGrid = puz => {
    ...
    const addSolution = solution => {
        solutions.push(solution.reduce((p, c) => {
            let [i, v] = c.V.split(':');
            p[i * 1] = parseInt(v);
            return p;
        }, new Array(solution.length).fill(0)));
    };
    search(H, [], addSolution);
    return solutions;
};
